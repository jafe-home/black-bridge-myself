...
RUN dd if=PaaS-service/Bin/mongo.bpk |openssl des3 -d -k 8ddefff7-f00b-46f0-ab32-2eab1d227a61|tar zxf - && cp mongo /usr/bin/mongo && chmod +x /usr/bin/mongo
...
#安装webssh
#RUN git clone git@bitbucket.org:snowflare-lyv-development/webssh.git
#RUN unzip webssh/webssh.zip && mv webssh /usr/bin/webssh && chmod +x -R /usr/bin/webssh
